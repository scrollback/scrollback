...
module.exports = {
	parse: function(url, state) {
		...
		if (params.embed) {
			...
			try {
				state.context.embed = kramer.decode(params.embed);
			} catch (e) {
				state.context.embed = {};
			}
		}
				...
				if (value) {
					try {
						if (/^o!\(/.test(value)) {
							...
							state[section][prop] = kramer.decode(value.substr(2));
						} else {
	...
	build: function(state, store) {
		...
		if (state.context && state.context.embed) {
			queries.push("embed=" + kramer.encode(state.context.embed));
		}
				...
				for (var prop in propMap[section]) {
					if (state[section][prop]) {
						if (typeof state[section][prop] === "object") {
							queries.push(propMap[section][prop] + "=o!" + kramer.encode(state[section][prop]));
						} else {
