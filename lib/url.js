...
module.exports = {
	parse: function(url, state) {
		...
		if (params.e || params.o || params.n || params.j) { state.context = state.context || {}; }
		if (params.e) { state.context.embed  = params.e; state.context.env = "embed"; }
		if (params.o) { state.context.origin = params.o; }
		if (params.n) { state.context.nick   = params.n; }
		if (params.j) { state.context.jws    = params.j; }
		
		if (params.d || params.v || params.ds) { state.nav = state.nav || {}; }
		if (params.d)  { state.nav.dialog      = params.d; }
//		if (params.ds) { state.nav.dialogState = params.ds; }
	...
	build: function(state, store) {
		...
		case "chat":
			...
			path = "/" + state.nav.room + "/" + (state.nav.thread ? state.nav.thread : "all") + (title ? "/" + format.urlComponent(title) : "");
			break;
		default:
			path = "/";
		}
		...
		if (state.context && state.context.env !== "web") {
			...
			if (state.context.nick)   params.n = state.context.nick;
			if (state.context.jws)    params.j = state.context.jws;
		}
		
		if (state.nav) {
			if (state.nav.dialog) {
				params.d = state.nav.dialog;
//				if (state.nav.dialogState) { params.ds = state.nav.dialogState; }
			}
