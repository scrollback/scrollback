...
function removeBlockingChracterFromMiddle()
{
	/*Iterating over each key in di*/
	var key;
	for(key in di)
	{
		/*getting list for middle Characters*/
		var middleList = di[key]['m'];
		var lastChar = key.substr(-1);
		
		var ch;
		/*making copy for iterating over the list*/
		var temp = middleList
		
		for(ch in temp)
		{
			/*bigram here is all the next possible combination */
			var bigram = lastChar + middleList[ch] ;
			
			/*if bigram is not in the di then removing that character from the list of the middle*/
			if(!(bigram in di))
			{
				middleList.splice(ch,1);
				
			}
		}
		/*assigning new list of the middle character to the specific key*/
		di[key]['m'] = middleList;
	}
}

removeBlockingChracterFromMiddle();
