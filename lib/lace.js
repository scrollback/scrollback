    ...
    return function() {
        ...
        _this.events = {
            ...
            core: function(options) {
                ...
                if (options.support && $element.is(":visible") && document.hasFocus()) {
                    ...
                    }).css({ transition: ".15s ease" }).css(options.css).data("_this.animate", true);

                    // Fix event not firing when window not focused
                    $(window).on("blur._this.animate", function() {
                        onanimate();
                    });
                } else {
                    options.action.call($element);
        ...
        _this.animate = {
            ...
            fadeout: function(element, action) {
                ...
                if ($.fn.velocity) {
                    ...
                    _this.events.transitionend({ opacity: 0 }, element, action);
                }
            }
        };
        ...
        _this.modal = {
            ...
            init: function() {
                ...
                $(document).on("click", ".modal-remove", _this.modal.hide);
        ...
        _this.popover = {
             ...
             */
            init: function() {
                if (_this.popover.init.done) {
                    return;
                }

                $(document).on("click", ".popover-layer", _this.popover.hide);

                _this.popover.init.done = true;
            },
        ...
        _this.alert = {
             ...
             */
            init: function() {
                if (_this.alert.init.done) {
                    return;
                }

                $(document).on("click", ".alert-remove", function() {
                    _this.alert.hide($(this).parent($(".alert-bar")));
                });

                _this.alert.init.done = true;
            },
