...
describe('validate test', function() {
	...
	it('room starts with number', function() {
		assert.equal(validate("1ha"),false,"2 char is allowed");
		assert.equal(validate("1345678hhhhhhh"),false,"1 char is allowed");
		assert.equal(validate("1345678hhhhhhh", true), "hhhhhhh","Invalid sanitized value");
		assert.equal(validate("1ha", true),"ha-", "Invalid sanitized value");
	});
});
