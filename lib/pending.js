...
function keyGeneration(query) {
	...
	switch (query.type) {
		...
		case 'getThreads':
			...
			if (query.before) keyParts.push("before");
			if (query.after) keyParts.push("after");
			break;
		case "getUsers":
		case "getRooms":
		case "getEntities":
...
function hasPendingQuery(store, query) {
	var key = keyGeneration(query);
	var queryDone = store.get("pendingQueries", key);

	if (!queryDone) return false;

	if (query.type === "getTexts" || query.type === "getThreads") {
		if (query.before && query.before > queryDone.before) return false;
		if (query.after && query.after > queryDone.after) return false;
	}
