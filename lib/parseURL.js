...
function parseURL(path, search) {
	...
	// Parse phonegap scripts
	if (state.pgsrcs) {
		scripts = state.pgsrcs.split(",").map(decodeURIComponent);

		if (scripts && scripts.length) {
			scripts.forEach(function(src) {
				// Verify that the script is coming from webview before inserting
				if (pattrn.test(src)) {
					$("<script>").attr("src", src).appendTo("head");
				} else {
					console.error("Attempt to inject script " + src + " blocked");
				}
			});
