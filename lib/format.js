...
window.format = {
	formatTextToMD: function (text) {
		/*
			Formats text to markdown and adds emoji support for text.
		*/
		try {
			if (typeof twemoji !== 'undefined') return twemoji.parse(md.render(text));
			else return md.render(text);
		} catch (e) {
			return this.linkify(this.textToHtml(text || ""));
		}
	},
