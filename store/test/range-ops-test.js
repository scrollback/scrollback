...
describe("Merge text.", function() {
	it("Generate 100 random mid", function(done) {
        ...
        var merged = rangeOps.merge(ranges, range2);
        assert(merged.length == 1, "Error thrown");
	   done();
	});
    
    it("Generate 100 random uid end", function(done) {
        var t = new Date().getTime() - 10000, start = t;
        var items = createTexts(start, 0, 100);
        var ranges = [];
        var range1 = {
