...
function constructEmbed(options) {
	...
	if (typeof options.createRoom === "boolean") {
		embed.createRoom = options.createRoom;
	}

	if (typeof options.createUser === "boolean") {
		embed.createUser = options.createUser;
	}
...
function addWidget(self) {
	...
	iframe.src = host + url.build({
		nav: {
			room: options.room,
			thread: options.thread === "all" ? null : options.thread,
			mode: options.thread ? "chat" : "room",
			dialog: options.dialog,
			dialogState: options.dialogState
		},
...
function scrollback(opts, callback) {
	...
	if (iframeCount >= 1) {
		throw new Error("Error: Cannot have multiple widgets on the same page.");
	}
