...
module.exports = function(core) {
	core.on('conf-show', function(tabs, next) {
		...
		var users = relatedUsers.filter(function(user){return user.transitionType === 'request'});
	...
	$(document).on('click', '.auth-admit', function(e) {
		...
		var room = $(element).data("room"), user = $(element).data("user");
		console.log(room, user);
		core.emit("admit-up", {to:room, ref:user, role:'follower'});
	});
};
