...
module.exports = {
    populateMembers: function (room) {
        ...
        var members;
        libsb.emit("getUsers", {memberOf: room}, function (err, data) {
    ...
    populateOccupants: function (room) {
        var occupants;
        libsb.emit("getUsers", {occupantOf: room}, function (err, data) {
            occupants = data.results;
            this.putOccupants(room, occupants);
        });
    },
	getMembers: function (room, member) {
		if (/^guest-/.test(member.id)) {
			return []; // guest cannot be a member
		}
		if (typeof member !== "undefined") {
			// return the single member as an Array
            if (!roomMemberList[room].hasOwnProperty(member.id)) {
                return [];
            }
			return [roomMemberList[room][member.id]];
		} else {
	...
	getOccupants: function (room, occupant) {
        if (typeof occupant !== "undefined") {
            ...
            if (!roomOccupantList[room].hasOwnProperty(occupant.id)) {
                // the room does not have this occupant
                return [];
            } else {
               // return occupant object 
                return [globalOccupantList[occupant.id]];
            }
        } else {
