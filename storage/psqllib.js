...
exports.paramize = function paramize (query) {
	var ixs = {}, sql, vals=[];
	...
	function paren(p, v, wrap) {
		...
		if(Array.isArray(v)) {
			return (wrap? "(": "") +
				v.map((iv, ix) => paren(p + "-" + ix, iv, true)).join(", ") +
				(wrap? ")": "");
		} else {
			return "$" + getIndex(p, v);
		}
	...
	sql = query.$.replace(/\$\{(\w+)\}/g, (m, p) => "$" + getIndex(p, query[p]))
		.replace(/\$\((\w+)\)/g, (m, p) => paren(p, query[p]));
	
	return { q: sql, v: vals };
};

exports.join = function join (parts, delim) {
	delim = delim || " ";
	
	var q = {
		$: parts.map(part => (part.$ || part)).join(delim)
	};
...
exports.insert = function (source, object) {
	var columnNames = Object.keys(object);
	
	return {
		$: "INSERT INTO " + source + " (" + columnNames.join(",") + ") VALUES ($(values))",
		values: columnNames.map(name => object[name])
	};
};
